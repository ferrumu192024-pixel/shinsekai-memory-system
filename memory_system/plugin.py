...
class MemorySystemPlugin(PluginBase):
    ...
    def initialize(
        ...
        # ── 1. 注册 LLM 工具 ──────────────────────────────────────
        # 将归档搜索、日记读取等功能注册为 AI 可调用的工具
        self._register_tools(register)

        # ── 2. 注入系统提示词规则 ─────────────────────────────────
        # 通过 OutputContractPatch 向角色模板追加回忆相关的规则
        self._patch_prompt(register)

        # ── 3. 挂载后端管理器到宿主 ──────────────────────────────
        # DiaryManager 和 RandomRecallManager 需要在 llm_manager 中初始化
        # 这一部分依赖于核心文件的改动，在插件中无法直接完成，
        # 因此保留 llm_manager.py 中的初始化代码。
        # 插件只负责工具注册和提示词注入。

    # ── 工具注册 ──────────────────────────────────────────────────

    def _register_tools(self, register: PluginCapabilityRegistry) -> None:
        """将 archive 和 diary 工具注册到 ToolManager"""
        # 导入工具注册函数
        from .tools.archive_tools import _register_archive_tools
        from .tools.diary_tools import _register_diary_tools

        # 使用 register_llm_tool 回调方式注册
        def register_archive(tm):
            _register_archive_tools()

        def register_diary(tm):
            _register_diary_tools()

        register.register_llm_tool(register_archive)
        register.register_llm_tool(register_diary)
